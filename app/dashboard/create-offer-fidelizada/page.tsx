...
import { getOfferById, getSessionById, createSession, applyFixedTerm, updateOfferDates } from "@/lib/api-client"
...
function CreateOfferFidelizadaContent() {
  ...
  // A opção de fidelidade está sempre ativa nesta versão
  ...
  const handleCreateSession = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Ativar a fidelização para a oferta recorrente
      try {
        // Aplicar fidelização à oferta recorrente
        await applyFixedTerm({
          offerId: session.recurrentOfferId,
          isFixedTermOffer: true
        })
        console.log("Fidelização aplicada com sucesso à oferta recorrente");
      } catch (fidelizacaoError) {
        console.error("Erro ao aplicar fidelização à oferta recorrente:", fidelizacaoError);
        toast({
          title: "Atenção",
          description: "Houve um problema ao ativar a fidelização.",
          variant: "destructive"
        });
        // Não bloqueamos o fluxo principal se houver erro
      }
  ...
  const handleShowSession = async () => {
    ...
    try {
      ...
      // Exibir os dados da sessão exatamente como retornados pela API
  ...
  const handleCloseSession = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/sales/sessions/${sessionData.id}/close`, {
        ...
        body: JSON.stringify({}) // Não modificar o payload
      })
            ...
            <Card className="mb-6">
              <CardContent className="pt-6">
                <div className="flex items-center space-x-2">
                  ...
                  />
                  <div>
                    <label htmlFor="loyalty" className="font-medium">Fidelizado</label>
                    <p className="text-sm text-muted-foreground">Ativa a fidelização do cliente nesta oferta</p>
                  </div>
                </div>
              </CardContent>
            </Card>
...
export default function CreateOfferFidelizadaPage() {
  return (
    <Suspense fallback={<div className="flex justify-center py-8">
      <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>
    </div>}>
      <CreateOfferFidelizadaContent />
    </Suspense>
  )
} 