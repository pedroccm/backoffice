...
export async function POST(
  ...
  try {
    ...
    console.log(`Aplicando duração ${body.offerDurationId} à oferta ${offerId}`);
    
    // Fazer a requisição para a API externa
    const apiResponse = await fetch(`${SALES_API_URL}/offers/${offerId}/duration`, {
      method: 'POST',
      headers: {
    ...
    if (!apiResponse.ok) {
      ...
      if (process.env.NODE_ENV === 'development') {
        // Vamos simular a aplicação de uma duração para desenvolvimento
        const mockOffer = {
          id: offerId,
          items: [],
          ...
          subtotal: 0,
          total: 0,
          offerDurationId: body.offerDurationId,
      ...
      return NextResponse.json(
        { error: `Falha ao aplicar duração à oferta: ${apiResponse.status}` }, 
    ...
    if (process.env.NODE_ENV === 'development') {
      const offerId = params.id;
      const body = await request.json().catch(() => ({ offerDurationId: 'duration-1' }));
